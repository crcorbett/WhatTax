...
| Configuration | Change required | `tools/governance/tsconfig.json`, `oxlint.config.ts`, and existing exact boundary allowlists if the new runtime needs them. | Typecheck and lint pass without broad exemptions, nested config, unsafe casts, or user-specific paths. |
