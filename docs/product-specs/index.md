...
| Spec | Status | Notes |
| --- | --- | --- |
| [Doppler configuration governance](./doppler-configuration-governance.md) | Active | The edit-first PRD review accepted the ordered repository slices. The TaxKit-only bootstrap, PR 73 merge, merged-main Quality and approved automatic `pr-73` absent-stage teardown are proved. Hosted fork proof, Preview deployment, Production/rollback and legacy removal remain bounded. |
