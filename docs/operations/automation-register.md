...
fresh default-branch PR-close receipt before promotion. Plan v2 admits exactly
one native Website action. The provider-free Vite/workerd build is preflight
proof; the deployment-input digest binds the tracked Website source set rather
than claiming byte identity with Alchemy's internal build.
